...
OUTPUT_FORMATS = ('csv', 'tsv', 'json', 'yara', 'netflow', 'nw')

def getHandler(output_format):
    output_format = output_format.lower()
    if output_format not in OUTPUT_FORMATS:
        print("[WARNING] Invalid output format '%s' specified... using CSV" % output_format)
        output_format = 'csv'
...
class OutputHandler_nw(OutputHandler):
    def __init__(self):
        self.matches = {
                'URL': [],
                'Host': [],
    ...
    def print_match(self, fpath, page, name, match):
        if name in self.matches:
            print("name matches")
            self.matches[name].append(self.formats[name].format(match))
